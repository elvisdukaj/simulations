...
export {
	...
	private:
		explicit App(SDL_Window* window, vis::engine::Engine& engine)
				: window{window}, engine(engine), time_start{App::now()}, program{std::nullopt}, shape{} {
			initialize_video();

			shape = vis::opengl::create_regular_shape(vis::vec2{}, 0.5f, vis::vec4{}, 50);
		}
...
void main()
{
	...
	private:
		...
		std::optional<vis::opengl::GeometryShape> shape;
	};
